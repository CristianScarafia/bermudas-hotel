  ...
  return (
    <header className={`site-header ${overlay ? "header-overlay" : "header-solid"}`}>
      <div className="header-inner container">
        ...
        <a className="header-book" href="https://wa.me/542236076020?text=Hola%20Hotel%20Bermudas%2C%20quisiera%20consultar%20disponibilidad" target="_blank" rel="noreferrer">Reservar <span aria-hidden="true">↗</span></a>
  ...
  return (
    <footer className="footer">
      <div className="footer-main container">
        ...
        <div className="footer-column footer-reserve"><h2>Reservas</h2><p>Consultá disponibilidad y obtené atención personalizada.</p><a className="text-link light-link" href="https://wa.me/542236076020" target="_blank" rel="noreferrer">Escribir por WhatsApp <span>↗</span></a></div>
      </div>
...
export function WhatsAppButton() {
  return <a className="whatsapp" href="https://wa.me/542236076020?text=Hola%20Hotel%20Bermudas%2C%20quisiera%20consultar%20disponibilidad" target="_blank" rel="noreferrer" aria-label="Consultar disponibilidad por WhatsApp"><span>WhatsApp</span><b>✆</b></a>;
}

export function Icon({ name }: { name: string }) {
  const chars: Record<string, string> = { wifi: "⌁", coffee: "☕", snow: "❄", safe: "◇", tv: "▣", bed: "▰", bath: "◒", map: "⌖" };
  return <span className={`service-icon icon-${name}`} aria-hidden="true">{chars[name] ?? "•"}</span>;
}
